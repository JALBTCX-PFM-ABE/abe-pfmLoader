...
void compute_bin_values (DEPTH_RECORD *depth_record, BIN_RECORD *bin)
{
    ...
    double              sum_filtered, sum2_filtered;
    int16_t             validity;
...
void InitializeAreaFilter (PFM_DEFINITION *pfm_def)
{
    ...
    for (int32_t i = 0 ; i < 3 ; i++)
    {
        if (row[i] != NULL) 
        {
            for (int32_t j = 0 ; j < prev_width ; j++)
            {
        ...
        for (int32_t j = 0 ; j < pfm_def->open_args.head.bin_width ; j++)
        {
...
void AreaFilter (NV_I32_COORD2 *coord, BIN_RECORD *bin_record, PFM_DEFINITION *pfm_def, 
                 double bin_diagonal)
{
  int32_t                m = 0, n = 0, start_y, end_y, start_k, rc, sumcount, pointcount;
  uint32_t               validity;
  double                 depth, dx = 0.0, BinSigmaFilter, sum2, avgsum, stdsum, avg, std, slope;
  uint8_t                flat;
  ...
  if (coord->y != prev_coord_y && coord->y > 1 && coord->y < pfm_def->open_args.head.bin_height - 1)
    {
              ...
              if (row[2][i].dep != NULL)
                {
                  ...
                  for (uint32_t j = 0 ; j < row[2][i].bin.num_soundings ; j++)
                    {
      ...
      for (int32_t i = start_y, k = start_k ; i <= end_y ; i++, k++)
        {
          for (int32_t j = 0 ; j < pfm_def->open_args.head.bin_width ; j++)
            {
                  ...
                  if (!(bin_record->validity & PFM_DATA)) 
                    {
                      ...
                      recompute_bin_values_from_depth_index (pfm_def->hnd, bin_record, 0, row[m][n].dep);
